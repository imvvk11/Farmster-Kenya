class AppSettings(object):
    ...
    def __init__(self, prefix):
        ...
        assert (self.AUTHENTICATION_METHOD ==
                self.AuthenticationMethod.PHONE) #or self.UNIQUE_EMAIL
        assert (self.EMAIL_VERIFICATION !=
                self.EmailVerificationMethod.MANDATORY) \
            or self.EMAIL_REQUIRED
        if not self.USER_MODEL_PHONE_FIELD:
            assert not self.PHONE_REQUIRED
            assert self.AUTHENTICATION_METHOD \
                not in (self.AuthenticationMethod.PHONE)
                        #,self.AuthenticationMethod.PHONE_EMAIL)
    ...
    @property
    def PHONE_MIN_LENGTH(self):
        """
        Minimum username Length
        """
        return self._setting("PHONE_MIN_LENGTH", 1)

    @property
    def PHONE_BLACKLIST(self):
        """
        List of usernames that are not allowed
        """
        return self._setting("PHONE_BLACKLIST", [])
